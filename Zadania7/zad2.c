...
int main(void) {
    char *filename = "C:\\Users\\mbryla\\CLionProjects\\untitled\\Zadania_C\\c_lista2.txt";
    long total_sum = 0;
...
void find_digits(const char *str, int *first_digit, int *last_digit) {
    char word[6]; // max word length for digits is "three" which is 5 + null terminator
    int word_idx = 0;
    int i = 0;
            ...
            *last_digit = digit;
            ...
            if (word_idx >= 3) {
                ...
                if (digit != -1) {
                    ...
                    *last_digit = digit;
                    word_idx = 0; // reset word index
                }
            }
        } else {
            word_idx = 0; // reset word index if a non-alphabetic character is encountered
        }
        i++;
    }
}